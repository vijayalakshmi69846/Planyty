...
import { Mail, Lock, User, UserPlus, LogIn, ArrowLeft, Sparkles, Shield, AlertTriangle, CheckCircle, Eye, EyeOff } from 'lucide-react';
...
const SignUp = () => {
  ...
  const handlePasswordSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      await acceptInvitation(invitationToken, name, password);
    } catch (err) {
      setFieldErrors({ password: err.message || 'Failed to create account.' });
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-[#EED5F0] via-white to-[#A067A3] p-6">
        <div className="w-full max-w-md p-8 space-y-6 bg-white rounded-2xl border border-gray-200 shadow-[0_10px_40px_rgba(0,0,0,0.1)] hover:shadow-[0_20px_50px_rgba(0,0,0,0.15)] transition-all duration-500 animate-slide-in-left">
          ...
          <form onSubmit={handlePasswordSubmit} className="space-y-4 animate-stagger">
            ...
            <Button
              type="submit"
              className="w-full flex items-center justify-center bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-600 hover:to-pink-600 text-white font-bold py-4 rounded-xl transition-all duration-300 hover:scale-105 hover:shadow-xl shadow-lg mt-4"
