...
exports.sendCompanyInvitations = async (req, res) => {
  try {
    ...
    if (duplicate) return res.status(409).json({ success: false, message: 'Company or Email already exists.' });
    ...
    // 3. EXECUTE INVITATIONS
    // map() executes them, Promise.all() waits for all to finish
    const results = await Promise.all(invitationsList.map(invite => createAndSendInvitation(invite)));

    // 4. KAFKA LOGGING
    try {
      await sendActivityLog(0, 'COMPANY_ONBOARDED', 'Company', newCompany.id, { companyName });
...
exports.sendStep1Validation = async (req, res) => {
  try {
    ...
    if (existingCompany) return res.status(409).json({ success: false, message: 'Company name taken.' });
